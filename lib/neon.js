...
async function initSchema() {
  ...
  const schemaSql = `
    ...
    VALUES 
      ('free', 'Free', 0, 5, 0, '5 Send actions. Guest mode without account or saved settings.'),
      ('starter', 'Starter', 100, 50, 0, '50 Send actions per month. User saved login and settings data.'),
      ('growth', 'Growth', 200, 100, 0, '100 Send actions per month. Saved settings + use email templates.'),
      ('pro', 'Pro', 300, 300, 0, '300 Send actions per month. Use templates + make own email templates.'),
      ('lifetime', 'Lifetime', 1000, -1, 1, 'Unlimited Send actions forever. Lifetime access with custom templates & all features.')
...
async function getUserByIdFromNeon(id) {
  ...
  try {
    ...
    console.warn('[Neon Postgres] getUserByIdFromNeon error:', err.message);
    return null;
  }
}

async function getUserByEmailFromNeon(email) {
  ...
  try {
    ...
    console.warn('[Neon Postgres] getUserByEmailFromNeon error:', err.message);
    return null;
  }
}
...
async function saveUsageBalance(bal) {
  if (!process.env.DATABASE_URL) return;
  try {
    const userCheck = await query('SELECT id FROM users WHERE id = $1', [bal.user_id]);
    if (!userCheck || userCheck.rows.length === 0) return;

    const validPaidPlans = ['free', 'starter', 'growth', 'pro', 'lifetime'];
    const planId = (bal.plan_id && validPaidPlans.includes(bal.plan_id)) ? bal.plan_id : 'starter';
...
async function savePayment(payment) {
  if (!process.env.DATABASE_URL) return;
  try {
    const userCheck = await query('SELECT id FROM users WHERE id = $1', [payment.user_id]);
    if (!userCheck || userCheck.rows.length === 0) return;
    await query(`
