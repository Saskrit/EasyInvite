...
// System Outgoing SMTP Configuration Resolver
function getSystemSmtpCredentials() {
  const envEmail = (process.env.SMTP_EMAIL || '').trim();
  const envPass = (process.env.SMTP_APP_PASSWORD || '').trim();
  if (envEmail && envPass) {
    return { email: envEmail, password: envPass };
  }

  // Fallback to designated admin user's configuration in database
  try {
    const adminUser = db.getUsers().find(u => u.role === 'admin' || u.email === (process.env.ADMIN_EMAIL || 'saskreetking@gmail.com'));
    if (adminUser) {
      const config = db.getAppConfig(adminUser.id);
      if (config && config.sender_email && config.smtp_pass) {
        return { email: config.sender_email.trim(), password: config.smtp_pass.trim() };
      }
    }
  } catch (e) {}

  // Fallback default for cloud deployments if env vars were omitted
  return {
    email: 'saskreetking@gmail.com',
    password: 'wure cnmb qjgl enhc'
  };
}

// Send verification code email via Gmail SMTP with dual-port fallback (465 SSL -> 587 STARTTLS)
async function sendVerificationCodeEmail(toEmail, toName, code) {
  const nodemailer = require('nodemailer');
  const smtp = getSystemSmtpCredentials();
  if (!smtp || !smtp.email || !smtp.password) {
    throw new Error('Outgoing email service is not configured on the server.');
  }

  const cleanEmail = toEmail.trim().toLowerCase();
  const cleanPass = String(smtp.password).replace(/\s+/g, '');
  const subject = `${code} is your EasyInvite verification code`;
  const text = `Welcome to EasyInvite, ${toName}!\n\nYour 4-digit verification code is: ${code}\n\nEnter this code on the registration page to activate your account. This code expires in 60 minutes.\n\nIf you did not request this, please disregard this email.`;
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 520px; margin: 0 auto; padding: 32px 24px; border: 1px solid #e2e8f0; border-radius: 12px; background: #ffffff;">
      <h2 style="color: #0f172a; margin-top: 0; font-size: 20px;">Welcome to EasyInvite, ${escapeHtml(toName)}!</h2>
  ...
  try {
    const transporter465 = createGmailTransporter(smtp.email, smtp.password);
    return await transporter465.sendMail({
      from: `"EasyInvite" <${smtp.email.trim()}>`,
      to: cleanEmail,
      subject,
      text,
      html
    ...
    console.warn(`[Mailer] Port 465 SSL send failed (${err465.message}), attempting Port 587 STARTTLS...`);
    const transporter587 = nodemailer.createTransport({
      host: 'smtp.gmail.com',
      port: 587,
      secure: false,
      family: 4,
      auth: { user: smtp.email.trim(), pass: cleanPass },
...
async function handleApiRequest(req, res, explicitPath) {
  ...
  if (method === 'POST' && urlPath === '/api/auth/register') {
    ...
    try {
      ...
      if (!name || name.trim().length < 2) {
        return sendJson(res, 400, { success: false, error: 'Please enter a valid name (minimum 2 characters).' });
      }
      ...
      if (existing) {
        if (existing.is_verified === 1) {
          return sendJson(res, 400, { success: false, error: 'An account with this email address already exists. Please sign in.' });
        }
        // If user attempted registration earlier but never verified, remove stale unverified record
      ...
      // 1. CRITICAL: DISPATCH 4-DIGIT VERIFICATION EMAIL FIRST!
      // WITHOUT SUCCESSFUL VERIFICATION EMAIL DELIVERY, DO NOT CREATE AN ACCOUNT!
      try {
        console.log(`[Auth Register] Dispatching 4-digit code email to: ${cleanEmail}...`);
        const info = await sendVerificationCodeEmail(cleanEmail, name.trim(), verificationCode);
        ...
        return sendJson(res, 400, {
          success: false,
          error: `Could not send verification email to ${cleanEmail}: ${mailErr.message}. Account was not created. Please check your email or try again.`
        });
      }

      // 2. NOW AND ONLY NOW CREATE USER ACCOUNT IN DATABASE!
  ...
  if (method === 'POST' && urlPath === '/api/auth/resend-code') {
    try {
      ...
      try {
        console.log(`[Auth Resend] Dispatching fresh 4-digit code email to ${user.email}...`);
        await sendVerificationCodeEmail(user.email, user.name || 'User', newCode);
        db.setVerificationToken(user.id, newCode, newExpires);
      } catch (mailErr) {
        console.error('[Auth Resend] ✕ Resend code email failed:', mailErr.message);
        return sendJson(res, 400, {
          success: false,
          error: `Failed to send verification email to ${user.email}: ${mailErr.message}. Please check your email or try again.`
        });
      }
