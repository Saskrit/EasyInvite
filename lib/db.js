...
const DEFAULT_PLANS = [
  {
    ...
    sends_included: 10,
    is_unlimited: 0,
    description: '10 Send actions. Ideal for initial closed testing outreach.'
  },
  {
    id: 'growth',
    name: 'Growth',
    ...
    sends_included: 40,
    is_unlimited: 0,
    description: '40 Send actions. Most popular for regular closed testing builds.'
  },
  {
    id: 'pro',
    name: 'Pro',
    ...
    sends_included: 60,
    is_unlimited: 0,
    description: '60 Send actions. Best value for active dev teams.'
  },
  {
    id: 'lifetime',
    name: 'Lifetime',
    ...
    sends_included: -1,
    is_unlimited: 1,
    description: 'Unlimited Send actions forever. One-time payment, zero limits.'
  }
];
...
class JsonDatabase {
  ...
  createUser({ name, email, passwordHash, role = 'user', isVerified = 0, verificationToken = null, verificationTokenExpires = null }) {
    const cleanEmail = email.trim().toLowerCase();
    ...
    // Initialize usage balance (Free / 0 balance until a plan is purchased)
    this.data.usage_balances[String(nextId)] = {
      user_id: nextId,
      plan_id: 'free',
      sends_remaining: 0,
      is_lifetime: 0,
      total_sends_purchased: 0,
  ...
  verifyUserEmail(token) {
    ...
    const user = this.data.users.find(u => u.verification_token === cleanToken);
    if (!user) return null;
   ...
   * If user has Lifetime access, no deduction occurs but reservation succeeds.
  ...
  reserveSendUse(userId) {
    ...
    // Lifetime accounts have unlimited sends
    if (balance.is_lifetime === 1) {
  ...
  // --- Payments & Purchases ---
  createPayment({ userId, planId, amountNpr, sendsToCredit, isLifetime, referenceNumber, screenshotPath = null }) {
